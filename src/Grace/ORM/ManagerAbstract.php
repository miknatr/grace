...
abstract class ManagerAbstract implements ManagerInterface {
    ...
    public function __construct(InterfaceConnection $sqlReadOnly, CRUDInterface $crud, Dispatcher $eventDispatcher,
    ...
    public function commit() {
        foreach ($this->unitOfWork->getNewRecords() as $record) {
            ...
            $changes   = $this->getMapper($className)->convertRecordArrayToDbRow($record->asArray());
            $this->crud->insertById($className, $record->getId(), $changes);
        }
        foreach ($this->unitOfWork->getChangedRecords() as $record) {
            $className = $this->nameProvider->getBaseClass(get_class($record));
            $changes   =
                $this->getMapper($className)->getRecordChanges($record->asArray(), $record->getDefaultFields());
            if (count($changes) > 0) {
